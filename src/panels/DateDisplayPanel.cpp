...
DateDisplayPanel::DateDisplayPanel(wxWindow *parent) : SettingsPanel(parent) {
...
void DateDisplayPanel::LoadSettings() {
  ...
  m_localTimezone->SetValue(
      config.ReadBool("/Settings/LocalTimeDisplay", true));
  m_senderTimezone->SetValue(
      config.ReadBool("/Settings/SenderTimeDisplay", false));
  m_fixedFormat->SetValue(config.ReadBool("/Settings/FixedDateDisplay", true));
  m_ageSensitive->SetValue(
      config.ReadBool("/Settings/AgeSensitiveDateDisplay", false));
  m_fixedDateFormat->SetValue(
      config.Read("/Settings/FixedDateFormat", "%1 %2"));
  m_recentDateFormat->SetValue(config.Read("/Settings/RecentDateFormat", "%1"));
  m_oldDateHours->SetValue(
      wxString::Format("%ld", config.ReadLong("/Settings/OldDateHours", 24)));
  m_oldDateFormat->SetValue(config.Read("/Settings/OldDateFormat", "%2"));
  m_ancientDateHours->SetValue(wxString::Format(
      "%ld", config.ReadLong("/Settings/AncientDateHours", 168)));
  m_ancientDateFormat->SetValue(
      config.Read("/Settings/AncientDateFormat", "%2"));
}
