...
DisplayPanel::DisplayPanel(wxWindow *parent) : SettingsPanel(parent) {
...
void DisplayPanel::LoadSettings() {
  ...
  m_showLargeButtons->SetValue(
      config.ReadBool("/Settings/ShowLargeButtons", false));
  m_showStatusBar->SetValue(config.ReadBool("/Settings/ShowStatusBar", true));
  m_showCategoryIcons->SetValue(
      config.ReadBool("/Settings/ShowCategoryIcons", true));
  m_mdiTaskbar->SetValue(config.ReadBool("/Settings/MDITaskbar", true));
  m_autoDownloadGraphics->SetValue(
      config.ReadBool("/Settings/FetchInlineContent", false));
  m_displayImagesInline->SetValue(
      config.ReadBool("/Settings/DisplayImageAttachmentsInline", false));
  m_displayEmoticons->SetValue(
      config.ReadBool("/Settings/DisplayEmoticonAsPictures", false));
  m_minimizeToTray->SetValue(
      config.ReadBool("/Settings/ShowOnlyInTray", false));
  m_alwaysShowTrayIcon->SetValue(
      config.ReadBool("/Settings/AlwaysShowTrayIcon", false));
}
...
void DisplayPanel::SaveSettings() {
  ...
  config.Write("/Settings/FetchInlineContent",
               m_autoDownloadGraphics->GetValue());
  config.Write("/Settings/DisplayImageAttachmentsInline",
               m_displayImagesInline->GetValue());
  config.Write("/Settings/DisplayEmoticonAsPictures",
               m_displayEmoticons->GetValue());
  config.Write("/Settings/ShowOnlyInTray", m_minimizeToTray->GetValue());
  config.Write("/Settings/AlwaysShowTrayIcon",
               m_alwaysShowTrayIcon->GetValue());
